...
@section('content')
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold">Vendedoras</h1>
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
            ...
            <a href="{{ route('launches.admin.sellers.create') }}" class="inline-flex h-10 items-center justify-center rounded-md bg-brand-primary px-4 text-sm font-bold text-white hover:bg-brand-secondary">Nova vendedora</a>
        </div>
    </div>
    ...
    <div class="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white">
        @forelse($sellers as $seller)
            <div class="flex flex-col gap-3 border-b border-slate-100 p-4 last:border-b-0 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xs font-semibold {{ $seller->is_active ? 'text-emerald-600' : 'text-red-600' }}">{{ $seller->is_active ? 'Ativa' : 'Inativa' }}</p>
                </div>
        ...
        @empty
            <div class="p-6 text-sm text-slate-500">Nenhuma vendedora cadastrada.</div>
        @endforelse
    </div>
